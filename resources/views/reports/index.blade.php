    ...
    $reportCategories = [
        ...
        [
            ...
            'reports' => [
                ...
                [
                    'type' => 'supplier_purchase_bills',
                    'label' => __('messages.Supplier Purchase Bills'),
                    'icon' => 'store',
                ],
            ],
        ],
        ...
        [
            ...
            'color' => 'rose',
            'reports' => [['type' => 'expenses', 'label' => __('messages.Expenses Report'), 'icon' => 'expense']],
        ],
    ];
...
<x-app-layout>
    ...
    <div x-data="reportsApp()" class="flex h-[calc(100vh-4rem)] overflow-hidden bg-gray-50"
        dir="{{ $isRTL ? 'rtl' : 'ltr' }}">

        {{-- ══ LEFT SIDEBAR ════════════════════════════════════════════════ --}}
        <aside
            class="w-64 flex-shrink-0 bg-white border-{{ $isRTL ? 'l' : 'r' }} border-gray-200 overflow-y-auto no-print">
            <div class="p-3">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div class="print-only hidden print:block mb-6 border-b-2 border-gray-800 pb-4">
                        ...
                        <template x-if="activeType === 'customer_statement' && results && results.meta">
                            <div class="mb-4">
                                <h1 class="text-2xl font-bold text-gray-900">{{ __('messages.Customer Statement') }}
                                </h1>
                                <div class="mt-2 text-sm text-gray-700 space-y-0.5">
                                    <p><strong>{{ __('messages.Customer') }}:</strong> <span
                                            x-text="results.meta.name"></span></p>
                                    <p x-show="results.meta.phone"><strong>{{ __('messages.Phone') }}:</strong> <span
                                            x-text="results.meta.phone"></span></p>
                                    <p x-show="results.meta.balance != 0">
                                        ...
                                        <span
                                            x-text="results.meta.balance > 0 ? ' ({{ __('messages.owes') }})' : ' ({{ __('messages.credit') }})'"></span>
                                    </p>
                                </div>
                                <p class="text-gray-500 text-xs mt-2" x-text="printSubtitle"></p>
                                <p class="text-gray-400 text-xs">{{ __('messages.Generated at') }}:
                        ...
                        <div class="p-5">
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                ...
                                <template x-if="hasField('date')">
                                    <div>
                                        <label
                                            class="block text-xs font-medium text-gray-600 mb-1">{{ __('messages.Date To') }}</label>
                    ...
                    <template x-if="error">
                        <div
                            class="bg-red-50 border border-red-200 rounded-xl px-5 py-4 text-red-700 text-sm flex items-center gap-2 no-print">
                    ...
                    <template x-if="results">
                        <div class="space-y-4" id="report-results">
                            ...
                            <template x-if="results.rows.length >= 1000">
                                <div
                                    class="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 text-amber-700 text-xs no-print">
                                            ...
                                            <template x-if="results.rows.length === 0">
                                                <tr>
                                                    <td :colspan="activeColumns.length"
                                                        ...
                                                        <svg class="w-10 h-10 mx-auto mb-2 text-gray-200"
                                                ...
                                                <tr :class="idx % 2 === 0 ? '' : 'bg-gray-50/60'"
                                                    ...
                                                    <template x-for="col in activeColumns" :key="col.key">
                                                        <td class="px-3 py-2 whitespace-nowrap"
                                                            x-html="formatCell(row[col.key], col.type ?? 'text', row)">
                                                        </td>
                                                    </template>
                                                </tr>
                                            </template>
                ...
                return {
                    ...
                    buildSummaryCards(summary) {
                        ...
                        if (summary.total !== undefined) {
                            cards.push({
                                label: _trans.totalAmount,
                                value: this.fmtCurrency(summary.total),
                                color: 'text-indigo-700'
                            });
                        }
                        if (summary.total_price !== undefined) {
                            cards.push({
                                label: _trans.totalSales,
                                value: this.fmtCurrency(summary.total_price),
                                color: 'text-indigo-700'
                            });
                        }
                        if (summary.profit !== undefined) {
                            cards.push({
                                label: _trans.totalProfit,
                                value: this.fmtCurrency(summary.profit),
                                color: 'text-emerald-700'
                            });
                        }
                        if (summary.total_owing !== undefined) {
                            cards.push({
                                label: _trans.totalOwing,
                                value: this.fmtCurrency(summary.total_owing),
                                color: 'text-rose-700'
                            });
                        }
                        if (summary.total_credit !== undefined) {
                            cards.push({
                                label: _trans.totalCredit,
                                value: this.fmtCurrency(summary.total_credit),
                                color: 'text-blue-700'
                            });
                        }
                        return cards;
                    ...
                    buildPrintSubtitle() {
                        const parts = [];
                        if (this.hasField('date') && this.form.from) parts.push(
                        `{{ __('messages.From') }}: ${this.form.from}`);
                    ...
                    return '<span class="text-gray-300">—</span>';
                        ...
                        switch (type) {
                            ...
                            case 'bool_damage':
                                return value ?
                                    `<span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700">${_trans.damaged}</span>` :
                                    '';
                            case 'bool_return':
                                return value ?
                                    `<span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-700">${_trans.returned}</span>` :
                                    '';
                            case 'balance':
                                const n = parseFloat(value);
                                if (n > 0) return `<span class="font-medium text-rose-600">${this.fmtCurrency(n)}</span>`;
                                if (n < 0)
                                return `<span class="font-medium text-emerald-600">${this.fmtCurrency(Math.abs(n))}</span>`;
                                return `<span class="text-gray-400">0</span>`;
                            case 'note':
                                const str = String(value);
                                if (str.length > 60)
                                return `<span title="${this.escHtml(str)}" class="text-gray-600 cursor-help">${this.escHtml(str.substring(0, 60))}…</span>`;
                    ...
                    fmtDatetime(str) {
                        ...
                        return d.toLocaleDateString('{{ app()->getLocale() === 'ar' ? 'ar-SA' : 'en-US' }}', {
                            ...
                            ' ' + d.toLocaleTimeString('{{ app()->getLocale() === 'ar' ? 'ar-SA' : 'en-US' }}', {
                                hour: '2-digit',
                                minute: '2-digit'
                            });
                    },

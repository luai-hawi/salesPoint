                    ...
                    @else
                        ...
                        <div class="flex items-center space-x-1">
                            ...
                            <x-nav-link :href="route('products.out-of-stock')" :active="request()->routeIs('products.out-of-stock')"
                                ...
                                @endphp
                                @if($warningCount > 0)
                                    <span class="mx-2 mr-2 bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center">
                                        {{ $warningCount }}
                                    </span>
                                @endif
                            </x-nav-link>
                            ...
                            <div class="relative" x-data="{ moreOpen: false }">
                                ...
                                <div x-show="moreOpen" @click.away="moreOpen = false"
                                    ...
                                    @if(auth()->user()->role !== 'admin')
                                        @if((auth()->user()->role !== 'employee' || auth()->user()->hasPermission('manage_suppliers')) || (auth()->user()->role !== 'employee' || auth()->user()->hasPermission('manage_purchase_bills')))
                                        <!-- Suppliers Section -->
            ...
            <div class="px-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        ...
                        @if(Auth::user()->role === 'admin')
                            <div class="text-xs text-blue-600 font-semibold">{{ __('navigation.Administrator') }}</div>
                        @elseif(Auth::user()->role === 'shop_owner' || Auth::user()->role === 'restaurant' || Auth::user()->role === 'merchant')

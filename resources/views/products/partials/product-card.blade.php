...
<div class="product-card bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden transition-all duration-200 hover:shadow-md"
                ...
                <button type="submit"
                    ...
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="{{ $product->is_active ? 'M13 10V3L4 14h7v7l9-11h-7z' : 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' }}">
                        </path>
                    </svg>
                </button>
            </form>
    ...
    <div class="p-3">
        ...
        <div class="mb-3 p-2 bg-gray-50 rounded">
            <div class="flex items-center justify-between mb-1">
                ...
                <span
                    class="quantity-display text-sm font-bold {{ $isOutOfStock ? 'text-red-600' : ($isLowStock ? 'text-yellow-600' : 'text-green-600') }}">
                    {{ $product->quantity }}
                </span>
            </div>
        ...
        <div class="mb-3 text-center">
            ...
            <div
                class="text-sm font-semibold {{ $profitMargin > 20 ? 'text-green-600' : ($profitMargin > 10 ? 'text-yellow-600' : 'text-red-600') }}">
    ...
    <div class="px-3 pb-3 space-y-2">
        <!-- Add to Order Button -->
        <button
            class="add-to-order-btn w-full bg-purple-600 hover:bg-purple-700 text-white font-medium py-2 px-3 rounded text-sm transition-colors flex items-center justify-center"
        ...
        <div class="flex gap-1.5">
            ...
            <button
                class="delete-btn flex-1 bg-red-500 hover:bg-red-600 text-white text-xs font-medium py-1.5 px-2 rounded transition-colors flex items-center justify-center"
                data-product-id="{{ $product->id }}" data-product-name="{{ $product->name }}">
                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"

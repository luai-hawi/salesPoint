        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <h2 class="font-bold text-2xl text-gray-900 leading-tight flex items-center gap-3">
                    ...
                    @if ($totalExpired > 0)
                        <span
                            class="inline-flex items-center justify-center w-7 h-7 bg-red-600 text-white text-sm font-bold rounded-full">
                            {{ $totalExpired }}
                        </span>
                    @endif
                </h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white shadow-lg rounded-xl overflow-hidden">
                ...
                @if ($expiredLicenses->isEmpty())
                    <div class="p-8 text-center text-gray-400">
                        <svg class="w-12 h-12 mx-auto mb-3 text-green-400" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.License Expired') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Days Overdue') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Last Payment') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Subscription Cost') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                    ...
                                    <tr class="bg-red-50 hover:bg-red-100 transition-colors">
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="w-9 h-9 bg-gradient-to-br from-red-400 to-red-600 rounded-full flex items-center justify-center flex-shrink-0">
                                                    <span
                                                        class="text-white font-bold text-xs">{{ strtoupper(substr($user->name, 0, 2)) }}</span>
                                                </div>
                                                <div>
                                                    <div class="text-sm font-semibold text-red-800">{{ $user->name }}
                                                    </div>
                                                    <div class="text-xs text-gray-500">{{ $user->email }}</div>
                                                    <span
                                                        class="text-xs text-gray-400">{{ ucfirst(str_replace('_', ' ', $user->role)) }}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 text-sm text-gray-700">{{ $user->phone_number ?: '—' }}
                                        </td>
                                        ...
                                        <td class="px-4 py-3 text-sm text-gray-700">
                                            @if ($user->last_payment_months || $user->last_payment_amount)
                                                ...
                                                @if ($user->last_payment_amount)
                                                    <span
                                                        class="text-green-700 font-semibold ml-1">₪{{ number_format($user->last_payment_amount, 2) }}</span>
                                                @endif
                                            @else
                                        ...
                                        <td class="px-4 py-3 text-sm text-gray-700">
                                            @if ($user->subscription_cost)
                                                <span
                                                    class="font-semibold text-gray-800">₪{{ number_format($user->subscription_cost, 2) }}</span>
                            ...
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Phone') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.License Expires') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Days Left') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Last Payment') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Subscription Cost') }}</th>
                                ...
                                @foreach ($expiringSoonLicenses as $user)
                                    ...
                                    <tr class="bg-orange-50 hover:bg-orange-100 transition-colors">
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="w-9 h-9 bg-gradient-to-br from-orange-400 to-orange-600 rounded-full flex items-center justify-center flex-shrink-0">
                                                    <span
                                                        class="text-white font-bold text-xs">{{ strtoupper(substr($user->name, 0, 2)) }}</span>
                                                </div>
                                                <div>
                                        ...
                                        <td class="px-4 py-3 text-sm text-gray-700">
                                            @if ($user->last_payment_months || $user->last_payment_amount)
                                                @if ($user->last_payment_months)
                                                    <span
                                                        class="font-medium">{{ $user->last_payment_months }}mo</span>
                                                @endif
                                                @if ($user->last_payment_amount)
                                                    <span
                                                        class="text-green-700 font-semibold ml-1">₪{{ number_format($user->last_payment_amount, 2) }}</span>
            ...
            @if ($menuExpiredUsers->isNotEmpty())
                <div class="bg-white shadow-lg rounded-xl overflow-hidden">
                    <div class="px-6 py-4 border-b border-purple-200 bg-purple-50 flex items-center gap-2">
                        ...
                        <span
                            class="text-xs text-purple-500 ml-2">({{ __('messages.Separate online menu system') }})</span>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Phone') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Last Paid') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Last Amount') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        {{ __('messages.Expired On') }}</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
            ...
            @else
                <div class="bg-white shadow-lg rounded-xl overflow-hidden">
                    ...
                    <div class="p-8 text-center text-gray-400">
                        <p class="text-sm">
                            {{ __('messages.No expired menu subscriptions found or menu database unavailable.') }}</p>
                    </div>
                </div>
            @endif

        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg rounded-xl overflow-hidden">
                ...
                <form action="{{ route('admin.employees.store') }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        <label for="name"
                            class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.Full Name') }}</label>
                    ...
                    @if (auth()->user()->role !== 'employee')
                        <div>
                            <label for="shop_owner_id"
                                class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.Assign to Shop Owner') }}</label>
                            <select name="shop_owner_id" id="shop_owner_id" required
                                ...
                                @foreach ($shopOwners as $owner)
                                    <option value="{{ $owner->id }}"
                                        {{ old('shop_owner_id') == $owner->id ? 'selected' : '' }}>
                    ...
                    @else
                        <input type="hidden" name="shop_owner_id" value="{{ $shopOwners->first()->id ?? '' }}">
                    @endif
                    ...
                    @if ($shopOwners->count() > 0)
                        <div class="bg-blue-50 rounded-lg p-4">
                            <h4 class="text-sm font-medium text-blue-900 mb-2">
                                {{ __('messages.Available Shop Owners') }} ({{ $shopOwners->count() }}):</h4>
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <h3 class="text-sm font-medium text-yellow-800">
                                        {{ __('messages.No Shop Owners Available') }}</h3>
                                    <div class="mt-2 text-sm text-yellow-700">
                                        <p>{{ __('messages.You need to create shop owners first before adding employees') }}.
                                        </p>
                                    </div>
                                    <div class="mt-4">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 space-y-3">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <label class="flex items-center">
                                    ...
                                    <span
                                        class="ml-2 text-sm text-gray-700">{{ __('messages.Manage Products') }}</span>
                                </label>

                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="manage_bills"
                                        class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="manage_customers"
                                        class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    <span
                                        class="ml-2 text-sm text-gray-700">{{ __('messages.Manage Customers') }}</span>
                                </label>

                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="manage_suppliers"
                                        class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    <span
                                        class="ml-2 text-sm text-gray-700">{{ __('messages.Manage Suppliers') }}</span>
                                </label>

                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="manage_purchase_bills"
                                        class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    <span
                                        class="ml-2 text-sm text-gray-700">{{ __('messages.Manage Purchase Bills') }}</span>
                                </label>
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="manage_expenses"
                                        class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    <span
